...
<body>
  ...
  <script>
    ...
    function updateProgressBar() {
      const pct = Math.min(100, Math.round((languageStats.CorrectCount||0/15)*100));
      document.getElementById('progressBar').style.width = Math.round((correctCount/15)*100)+'%';
    ...
    function showSnippet() { showSnippetAdvanced(); }

    function showLevelThreeExplanation(selected) {
      const panel = document.getElementById('explanationPanel');
      const content = document.getElementById('explanationContent');
      if (!levelThreeMode) { panel.style.display='none'; return; }
      const correctLang = currentSnippet.lang;
      const explanationCorrect = currentSnippet.explanationCorrect||'';
      const explanationWrong = currentSnippet.explanationWrong||'';
      let html = `<p><strong>Correct Answer:</strong> ${correctLang}</p>`;
      if(explanationCorrect) html += `<p style="color:#2e8b57;"><strong>Why it's correct:</strong> ${explanationCorrect}</p>`;
      if(selected && selected!==correctLang){
        html += `<p><strong>Your Answer:</strong> ${selected}</p>`;
        if(explanationWrong) html += `<p style="color:#c0392b;"><strong>Why that's wrong:</strong> ${explanationWrong}</p>`;
      }
    ...
    function checkAnswer(selected) {
      ...
      if(selected===correctLanguage){
        ...
        if(explanationWrong) html+='<br><small style="color:#c0392b; display:block; margin-top:6px;">Why that is incorrect: '+explanationWrong+'</small>';
        if(explanationCorrect) html+='<br><small style="color:#2e8b57; display:block; margin-top:6px;">Answer: '+correctLanguage+' — '+explanationCorrect+'</small>';
        result.innerHTML=html;
        languageStats[selected].incorrect++;
      }
      updateStatsDisplay();
      if(levelThreeMode) showLevelThreeExplanation(selected);
    }

    function updateStatsDisplay() {
      let statsHtml='<div id="language-stats" style="margin-bottom:1em;"><h3>Language Stats</h3><table style="width:100%;text-align:center;border-collapse:collapse;"><tr><th>Language</th><th>Correct</th><th>Incorrect</th></tr>';
      languages.forEach(lang=>{ statsHtml+=`<tr><td>${lang}</td><td>${languageStats[lang].correct}</td><td>${languageStats[lang].incorrect}</td></tr>`; });
      statsHtml+='</table></div>';
    ...
    level2Btn.addEventListener('click',()=>{
      if(levelThreeMode){ levelThreeMode=false; level3Btn.classList.remove('active'); level3Btn.textContent='Level Three'; }
    ...
    level3Btn.addEventListener('click',()=>{
      ...
      if(levelThreeMode){ advancedMode=true; level2Btn.classList.remove('active'); level2Btn.textContent='Level Two'; }
    ...
    document.getElementById('nextLevelBtn')?.addEventListener('click',()=>{
      ...
      resetProgress();
      currentLevel++;
      if(currentLevel===2){ advancedMode=true; level2Btn.classList.add('active'); level2Btn.textContent='Level Two (on)
